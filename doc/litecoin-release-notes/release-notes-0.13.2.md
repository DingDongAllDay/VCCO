vccoin Core version 0.13.2 is now available from:

  <https://download.vccoin.org/vccoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/vccoin-project/vccoin/issues>
...
updates will be released anymore. Without security updates, using a vccoin
...
ECDSA signatures inside vccoin transactions now use validation using
...
Previous versions of vccoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of vccoin Core was to raise the default minimum
relay fee.

vccoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
vccoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running vccoind without having to do any manual configuration.
...
vccoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. vccoin Core will relay transactions with
...
entirely in vccoin Core 0.13, and it is at this time undecided whether the
...
vccoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
vccoin Core automatically creates a hidden service to listen on, without
manual configuration. vccoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if vccoin Core is listening, and
...
vccoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals vccoin
Core to use floating fees. By default, vccoin Core will use floating
fees.
...
(default: `0.0002` VCC/kB).

At all times, vccoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) VCC.
Furthermore, vccoin Core will never create transactions paying less than
...
`vccoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /vccoinrpc>
        ...
        # optional bypass vccoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< vccoinrpc:<password>
    </Location>
...
Restarting vccoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in vccoin.conf
...
For example:
    ...
    $ src/vccoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The vccoin Core code base has
...
- `vccoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `vccoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the vccoin scripting system that in combination with
...
vccoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of vccoin Core.
...
The two phases are distinct in the vccoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming vccoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for vccoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each vccoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of vccoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of vccoins being spent is exactly
...
- **Increased security for multisig:** vccoin addresses (both P2PKH addresses
  ...
  believed to be provided by vccoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of vccoin's current ledger (called the UTXO set),
  ...
  note that vccoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  vccoin users to individually opt-in to almost any change in the vccoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and vccoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the vccoin
client to vccoin Core 0.13.2 to signal for segwit support. When a
...
Since vccoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `vccoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `vccoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
